...
class UtilHelper
{
    ...
    public static function getPosition($name, $type = 0)
    {
        if($type = 0){
	...
	/**
	 * @deprecated
	 * @see UtilString::chineseSplit($str)
...
//         return $slice;
    ...
    /**
     * @deprecated
     * @see     	UtilString::pureStrSlice($str);
...
//    		UtilHelper::dump($pathinfo);
	 ...
	 */
	public static function writeToFile($content, $mode="w+", $line=__LINE__, $curfile=__FILE__, $filename = './public/log.txt',$separate = "\r\n")
	{
...
// 		$content = implode("\t\t", $content);
		...
		if (!file_exists($filename)) {
			UtilFile::createDir(dirname($filename));
		}
        ...
        {
        	$template = '[Datetime]'.date('y-m-d h:i:s')."\t".
					        	'[File]'.$curfile.'('.$line.')'.$separate.
					        	$content.$separate;
	 ...
	 * @param int $end
	 */
	public static function actionAnalysize($controller, $start, $end)
	{
		$log ='Action:'.Yii::app()->request->getUrl()."\t". date('Y-m-d h:i:s')."\tTime:".time()."\tStart:".$start. "\tEnd:".$end. "\tPass:".($end-$start);
		UtilHelper::writeToFile($log ,'a+', './public/time.txt');		
	}
